...
code_commune = st.text_input("Code INSEE commune", value="69383")  # Lyon 3e
...
from datetime import datetime
from prospection import ajouter_entree, charger_donnees
from dvf import get_mutations, get_parcelles_geojson
from map import generer_carte
from stats import stats_prospection, graphique_interet
from export import generer_pdf
from streamlit_folium import st_folium
...
geojson = get_parcelles_geojson(code_commune)
if "features" in geojson:
    m = generer_carte(geojson)
    st_folium(m, width=700, height=500)
else:
    st.error(geojson.get("error", "Impossible de charger les parcelles"))
